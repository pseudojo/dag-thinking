...
> **현재 버전**: v0.46 (138 tests) | 최종 갱신: 2026-06-13
...
| §4.2 릴리스 검증 파이프라인 | `prepare_release.py` — git/LOC/ruff/audit/tests/smoke 6종 | ✅ 준수 | v0.33 공급망 감사+CycloneDX SBOM 추가(§4.2-2, TD-8 해소). origin sync는 의도적 미적용 (CHANGELOG v0.33 참조) |
...
| TD-14 | `prepare_release.check_ruff`가 `src`만 검사 — `tests/`는 ruff 미적용(임포트 정렬·포맷 드리프트 축적, 예: `test_cleanup.py` I001) | 인프라 | 1 | 1 | 2 | 8 | v0.46 감사 도출. tests/ 린트 확장 시 기존 드리프트 일괄 정리 필요 |
