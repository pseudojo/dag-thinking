...
> **현재 버전**: v0.32 (128 tests) | 최종 갱신: 2026-06-12
...
├── tests/               — 행위 기준 8개 파일, 128개 테스트 (§12.1 아키텍처)
...
├── prepare_release.py   — §4.2 릴리스 검증 파이프라인 CLI, 5종 체크 (129 LOC)
...
| §4.2 릴리스 검증 파이프라인 | `prepare_release.py` — git/LOC/ruff/tests/smoke 5종 | ⚠️ 부분 준수 | v0.32 ruff 정적 분석 추가(§4.2-3). 취약점 감사(pip-audit)+SBOM 미구현 (TD-8) |
...
**해소 완료:** TD-1(v0.30 파일명), TD-2(v0.31 IMPROVEMENTS.md), TD-3(v0.32 `__all__` 제거), TD-4(v0.30 import fallback), TD-5(v0.30 ToolError), TD-7(v0.30 prepare_release.py)
