...
| 항목 | v0.1 | v0.2 |
|------|------|------|
| 툴 개수 | 5개 (`think`, `resolve`, `retrieve`, `invalidate`, `status`) | **1개** (`dag_headroom(action=...)`) |
...
응답:
{
  ...
  "restoration_manifest": {              ← 항상 포함
    "how_to_restore": "dag_headroom(action='restore', session_id='<id>', ccr_hash='<hash>')",
    "nodes": [
      {
        "name": "define_problem",
        "type": "Objective",
        "ccr_hash": "a3f8c2d1e5b9f0a2",
        "restore_cmd": "dag_headroom(action='restore', session_id='sess_01', ccr_hash='a3f8c2d1e5b9f0a2')"
      },
      ...
    ]
  }
...
```
입력 분기:
  len < 280자           → passthrough (noop)
  list 패턴 감지        → _compress_list()  : 중요도 기반 아이템 샘플링
  else                  → _compress_prose() : 문장 중요도 스코어링

스코어링:
  score = keyword_hits×1.5 + position_bonus + length_factor
...
```
[ 단일 진입점 ]
□ C01. MCP 툴이 dag_headroom 1개만 노출되는지 확인
...
[ 압축 ]
□ C10. 280자 미만 payload → compressed=NULL, tokens_saved=0
...
아래 설계 문서(hash: {{DESIGN_DOC_HASH}})에 따라 dag-headroom MCP 서버를 구현해 주세요.
설계 문서 원문이 필요하면 headroom_retrieve(hash="{{DESIGN_DOC_HASH}}")로 가져오세요.

## 구현 경로
D:\workspace\request-prompts\dag-headroom\
...
**Step 3 — server.py (단일 툴 dag_headroom)**
sequential_think로 먼저 구현 계획 수립 후 작성.
- action="think": depends_on → parent_context 자동 내장
- action="status": restoration_manifest 항상 포함
  restore_cmd 형식: dag_headroom(action='restore', session_id='...', ccr_hash='...')
- action="restore": hash 없음 → 목록, hash 있음 → 원본
- action="invalidate": cascade
...
## 절대 규칙
- 툴은 dag_headroom 1개만. resolve/retrieve/status를 별도 툴로 노출하지 말 것.
...
**조정 완료 요약:**
- 툴 5개 → **1개** (`dag_headroom(action=...)`)로 건너뛰기 문제 구조적 차단
...
- 구현 태스크 27개 + 검증 체크리스트 23개 + 구현 요청 프롬프트 완비# dag-headroom 설계 문서 v0.2
