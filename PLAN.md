...
| TD-9 | 툴 반환 outputSchema 미정의 | 코드 | 3 | 1 | 3 | 12 | 반환 타입 plain `dict` — action별 TypedDict 모델 정의 시 클라이언트가 응답 구조 사전 인지 가능. action 분기별 반환 구조가 달라 union 모델 설계 필요. v0.34 재평가: 외부 리뷰 3인 중 2인이 최우선 부채로 지목 — 다중 클라이언트/UI 통합 시 영향도 2→3 상향 |
| TD-10 | 패키지 버전 인상 대기 (pyproject 0.30 vs 문서 버전) | 문서 | 2 | 1 | 1 | 15 | MCP 서버 프로세스가 exe를 잠그는 동안 pyproject 편집 시 uv sync 실패. 서버 미기동 시점에 일괄 인상 — `action='info'`의 version이 그때까지 0.30 표시 |
| TD-11 | context_pressure 노드 수 기반 — 토큰 기반 전환 (§14.1) | 코드 | 3 | 2 | 2 | 20 | 외부 리뷰 2~3인 합의: 노드 수는 토큰 압박의 부정확한 대리값 (15×passthrough → false high, 7×1500자 → false low). `nodes.tokens_original/tokens_saved` 컬럼이 기 저장(R-3) — `SUM` 1-query 전환 가능. 임계값 재캘리브레이션 + C36/C37 테스트 개정 필요 |
...
**Phase 2 — 다음 사이클:**
- [ ] TD-11. context_pressure 토큰 기반 전환 (스펙 선행 — §14.1, 우선순위 20 최고)
- [ ] TD-10. pyproject 버전 일괄 인상 (0.30 → 현행 문서 버전, MCP 서버 미기동 시)
