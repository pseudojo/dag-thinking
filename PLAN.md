...
> **현재 버전**: v0.41 (130 tests) | 최종 갱신: 2026-06-13
...
│   │                      _compute_dag_health, dispatcher                            (416 LOC)
...
│   │                      _load_forward_edges, _has_cycle_graph, _cascade_invalidate  (152 LOC)
│   ├── compressor.py    — 순수 Python extractive 압축기                               (274 LOC)
│   └── __init__.py      — 빈 패키지 마커
├── tests/               — 행위 기준, 130개 테스트
...
- 최대 파일: `actions.py` 416 LOC ✅ (한도 내, 여유 84 LOC)
- 최소 파일: `db.py` 152 LOC ✅
...
> v0.32 행위 기준 8파일 스위트가 전 항목(C01~C66)을 커버 — 현행 130 tests 그린.
...
| TD-12 | INVALIDATED/ccr_store 보존 정책 부재 | 코드 | 1 | 2 | 3 | 9 | 상세 구현 스펙 → §10.1 |
...
- [ ] TD-12. 세션 보존/프루닝 정책 — §10.1 스펙 기준으로 구현
