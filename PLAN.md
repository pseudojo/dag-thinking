...
| v0.48 | 139 tests — CLEAN-15 해소(compressor `_compress_list`/`_compress_prose`의 동일 "score→top-k→원문순서 복원" 선택 로직 중복 → `_select_top_k()` 헬퍼로 단일화, DRY). 행위 불변(TestCompressList/TestCompressProse 안전망), compressor.py 274 LOC 불변. pyproject 0.45 유지(서버 연결 중) |

> **현재 버전**: v0.48 (139 tests) | 최종 갱신: 2026-06-13
