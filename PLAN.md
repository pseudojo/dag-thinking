...
| v0.48 | 139 tests — CLEAN-15 해소(compressor `_compress_list`/`_compress_prose`의 동일 "score→top-k→원문순서 복원" 선택 로직 중복 → `_select_top_k()` 헬퍼로 단일화, DRY). 행위 불변(TestCompressList/TestCompressProse 안전망), compressor.py 274 LOC 불변. pyproject 0.45→0.48 인상(서버 재기동 후 적용) |
| v0.49 | 139 tests (코드 불변 — 문서 일관성 정리, DOC-5) — 버전업 누적으로 드리프트된 마커·수치 일괄 정정(삭제 없음): PLAN 제목 v0.44→v0.49, §6/§9/§10 헤더 버전, §6 LOC 실측(actions 434→435·think 311→313)·테스트 수(138→139개)·누락 파일(test_cleanup.py) 보강, CHANGELOG/IMPROVEMENTS 범위·기준 버전 동기화, 폐기된 작업 섹션(§12–§15) 통합 안내 추가 |

> **현재 버전**: v0.49 (139 tests, 패키지 pyproject 0.48) | 최종 갱신: 2026-06-14
>
> **문서 구조 안내**: 과거 작업 노트 섹션 §12–§15(v0.32 스켈레톤 재구성 / v0.33 공급망 감사 / v0.34 외부 리뷰·ccr_hash 판정 / v0.35 재검증)는 위 버전 변경 내역 표와 `docs/CHANGELOG.md`·`docs/IMPROVEMENTS.md`로 통합됐다. CHANGELOG/IMPROVEMENTS에 남은 "PLAN §12~§15" 참조는 해당 버전 행과 개선 항목을 가리킨다.
...
│   │                      _restore_cmd, _compute_dag_health, _run_cleanup, dispatcher  (435 LOC)
│   ├── think.py         — think 액션 + 헬퍼: ThinkResult, _action_think,
│   │                      _validate_think_inputs, _compute_context_pressure 등       (313 LOC)
...
├── tests/               — 행위 기준, 139개 테스트 (8개 행위 파일)
│   ├── test_compressor.py / test_think.py / test_status.py / test_invalidate.py
│   ├── test_restore.py / test_dispatcher.py / test_mcp_protocol.py / test_cleanup.py
│   ├── tools/
...
## 9. MCP 표준 준수 현황 (v0.33 기준 — v0.49 재대조 확인)
...
> v0.48 재대조 (2026-06-13): 발굴 우선 감사 — compressor 선택 로직 중복(CLEAN-15) 식별·해소. `_compress_list`/`_compress_prose`가 보유하던 "score→top-k(floor 2)→원문순서 복원" 알고리즘을 `_select_top_k()`로 단일화(행위 불변, 기존 압축 테스트가 안전망). MCP 표준/소스 인터페이스 불변 — 신규 위반 0건. 굵직한 부채 소진 상태 재확인(잔여: TD-6 Docker·TD-13 eval·pyproject 인상=서버 재기동 필요). 139 그린.
> v0.49 정리 (2026-06-14): 코드 불변 — 버전업 누적 문서 드리프트 일괄 정정(DOC-5). 표준 준수 현황 변동 없음(재대조 위반 0건). PLAN 제목·§6/§9/§10 헤더 버전, §6 LOC 실측·테스트 수·누락 파일, CHANGELOG/IMPROVEMENTS 범위·기준 버전 동기화, §12–§15 통합 안내 추가. 패키지 버전은 v0.48에서 0.45→0.48로 인상됨(§1 v0.48 행). 참고: `uv run` 기반 ruff 훅은 연결 중 MCP 서버가 exe를 잠그면 sync 단계에서 실패 가능 — 훅에 `--no-sync` 추가로 회피(prepare_release.check_ruff와 동일 패턴).
...
| ~~CLEAN-15~~ | ~~compressor `_compress_list`/`_compress_prose` 선택 로직 중복 ("score→top-k→원문순서 복원" 12줄 ×2)~~ | 코드 | — | — | — | 해소 | v0.48: `_select_top_k(units, ratio, kw)` 헬퍼 추출 — 두 압축 경로가 공유. 행위 불변(기존 압축 테스트 안전망) |
